...
from langchain_core.documents import Document #LangChain’s standard container for a piece of text plus metadata.
from langchain_chroma import Chroma
from document_processor import ChunkWithMetaData
from config import *
from prompts import *

# Check whether OpenAI API Key is present or not
if OPENAI_API_KEY:
    print("Present", OPENAI_API_KEY[:10])
else:
...
class RagEngine:
     ...
     """
     def __init__(self, chroma_dir: str = CHROMA_PERSIST_DIR):
          ...
          # Initialise LLM for chat and re-ranking
          print("Initialising ChatOpenAI")
          self.llm = ChatOpenAI(
               model = VLM_MODEL,
     ...
     def index_chunks(self, chunks: List[ChunkWithMetaData], collection_name: str = COLLECTION_NAME):
          """
          Index document chunks into ChromDB
          ...
          # Create Lnagchain documents with metadata
          documents = []
          for chunk in chunks:
               doc = Document(
     ...
     def retrieve(self, query: str, top_k: int = INITIAL_NO_OF_CHUNKS) -> List[Document]:
        """
     ...
     def retrieve_and_rerank(self, query: str, 
                           initial_k: int = INITIAL_NO_OF_CHUNKS,
                           final_k: int = RE_REANKED_NO_OF_CHUNKS) -> List[Dict]:
        """
...
if __name__ == "__main__":
    ...
    pdf_path = PROJECT_ROOT / "Fund Facts - HDFC Income Fund - December 2025 [a].pdf"
    ...
    if os.path.exists(pdf_path):
        ...
        # Test chat
        print("\n--- Testing Chat ---")
        response, _ = rag.chat("Who manages the fund?")
